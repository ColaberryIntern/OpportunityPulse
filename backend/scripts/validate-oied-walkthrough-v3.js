...
const HTML_PATH = path.join(REPO_ROOT, 'docs', 'oied-feature-walkthrough-v3.html');
const ASSETS_DIR = path.join(REPO_ROOT, 'docs', 'oied-walkthrough-assets');
const FINAL_SHOT = path.join(ASSETS_DIR, 'walkthrough-v3-final.png');
...
(async () => {
  ...
  console.log('---V3 VALIDATION---');
  console.log('Fix sections:                  ', counts.fixSections);
  ...
  console.log(ok ? 'V3 AUDIT CLEAN' : 'V3 AUDIT FAILED');
  process.exit(ok ? 0 : 1);
})();
